...
client.on('message', message => {
  ...
  if (command === 'register') {
    ...
    fetch(`http://${process.env.API_HOST}:${process.env.API_PORT}/members`, {
      ...
      .then(([res, json]) => {
        ...
        return message.channel.send(`You are now registered. Check yourself out here: http://${process.env.SERVER_HOST}:${process.env.SERVER_PORT}/members/${name}`);
      })
  ...
  if (command === 'bio') {
    if (!args.length) message.channel.send('**Error:** You need to supply a bio');
    ...
    fetch(`http://${process.env.API_HOST}:${process.env.API_PORT}/members/${name}`, {
      ...
      .then(([res, json]) => {
        ...
        return message.channel.send(`Your bio has been set. Check yourself out here: http://${process.env.SERVER_HOST}:${process.env.SERVER_PORT}/members/${name}`);
      });
  }
});
